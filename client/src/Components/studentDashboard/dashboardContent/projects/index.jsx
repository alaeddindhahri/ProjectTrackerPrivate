        ...
        return (
            <div className="row">
                ...
                <div key={key} className="col-lg-3 col-xs-12 mt-4">
                        <div className={project.idInstructor?"card":"card text-white bg-secondary mb-3"}>
                            <h5 className="card-header">{project.name}</h5>
                            <div className="card-body">
                                <p className="card-text">{project.description}</p>
                                <p className="card-text">Github Link: {project.githubLink}</p>
                                <a href="" className="btn btn-primary" data-toggle="modal" data-target="#exampleModal" >Detail</a>
                                <EditModal id="#exampleModal" project={project}/>
                                <button className="btn btn-danger ml-3" onClick={() => { deleteProject(props.project._id); }}>Delete</button>
                            </div>
                        </div>
                    </div>
                )}
